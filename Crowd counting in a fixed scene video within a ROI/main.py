...
class Density_Estimation:
    ...
    def create_density_map(self, frame):
        ...
        p4        = (w, h*0.8)
        p5        = (w*0.5, h*0.55)
        roi       = np.array([[p1,p2,p3,p4,p5]], dtype= np.int32)
        
        zeros   = np.zeros(frame.shape[:2], dtype="uint8")
        mask    = cv2.fillPoly(zeros, pts=roi, color=255)
        frame2  = cv2.polylines(frame, pts=roi, isClosed=True, color=(0,0,255), thickness=2) 
        masked  = cv2.bitwise_and(frame, frame, mask=mask)
        
        # frame and thus roi_frame is array of uint8
        _input      = torch.Tensor(masked).permute(2,0,1).unsqueeze(0).to(device) 
        ...
        output      = cv2.addWeighted(frame2, 1, density_map, 1, 0)
        
        ## Stats
        output = cv2.putText(output, f'Total Count: {count}', (50, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, (0,0,255), 1, cv2.LINE_AA)
        return frame2, density_map, output
...
# url = 'https://youtu.be/pTWluE4q_PU'
url  = 'https://youtu.be/RllwWXor2g8'
